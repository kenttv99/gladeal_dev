...
- Uvicorn - ASGI-сервер.
- SQLAlchemy async - работа с базой данных в runtime.
...
- `api/utils/jwt_methods.py` - генерация JWT и проверка авторизации.
...
Роутеры клиента и исполнителя подключены с обязательной авторизацией. В auth-роутере публичными остаются только регистрация и логин.
...
- `POST /api/v1/auth/login` - авторизация по номеру телефона и выдача access token.
