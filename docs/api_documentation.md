...
Endpoint генерирует 4-значный код, сохраняет его в Redis с TTL `SMS_CALLS_CODE_TTL_SECONDS` и отправляет через ProstoSMS. Для `sms` используется текст `Код подтверждения: {code}`. Для звонка используется тот же payload с `route=pc`.
